...
export default function PivotTableApp() {
  ...
  /*
   * transporterNames maps a normalized Transporter ID -> DA name.
   * Loaded from a SEPARATE names file at runtime (the main data file only
   * has the Transporter ID). This lives only in the running app's state -
   * it is not persisted anywhere and is cleared when the page reloads.
   */
  const [transporterNames, setTransporterNames] = useState({});
  const [namesLoadedCount, setNamesLoadedCount] = useState(0);

  // Normalize an ID so matching is reliable regardless of stray spaces/case
  const normalizeId = (id) =>
    id === null || id === undefined ? '' : id.toString().trim().toUpperCase();

  // Look up the DA name for a Transporter ID (returns null if none loaded/matched)
  ...
  /*
   * handleNamesUpload: Parses the names file and builds the ID -> name map.
   * Auto-detects the Transporter ID column and the name column so it works
   * whether the header is "Transporter ID", "DA Name", "Driver", etc.
   */
    ...
    Papa.parse(file, {
      complete: (result) => {
        if (result.data && result.data.length > 0) {
          ...
          // Find the Transporter ID column
          const idCol =
          ...
          if (!idCol || !nameCol) {
            setError(
              'Names file needs a Transporter ID column and a name column. Columns found: ' +
                cols.join(', ')
            );
            return;
          }
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <div className="max-w-full mx-auto">
        ...
        <div className="bg-slate-900 rounded-xl shadow-lg p-6 mb-6 border border-slate-700">
          ...
          {/* Names file upload (Transporter ID -> DA name). Held only in-app, never stored. */}
          <label className="flex flex-col items-center justify-center w-full h-24 mt-4 border-2 border-dashed border-slate-600 rounded-xl cursor-pointer hover:bg-slate-800 hover:border-purple-500 transition-all duration-300">
            <div className="flex flex-col items-center text-center px-2">
              ...
              <span className="text-sm font-medium text-gray-300">Upload names file (Transporter ID → DA name)</span>
              <span className="text-xs text-gray-500">Optional — shows the DA name under each Transporter ID</span>
            </div>
